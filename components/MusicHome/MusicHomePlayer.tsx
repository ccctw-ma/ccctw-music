...
export default function MusicHomePlayer({ setIsHome, musicPlayer }) {
  ...
  const curMusicPlay = useRecoilValue(curMusicPlayState),
    setCurMusicPlay = useSetRecoilState(curMusicPlayState);
      ...
      <div className="w-full h-14 flex flex-row items-center bg-white">
        <div className="w-2/12" onClick={() => setIsHome(false)}>
          <img
            src={
              curMusic._coverUrl ||
              "http://mms0.baidu.com/it/u=2999215938,1867739424&fm=253&app=138&f=JPEG&fmt=auto&q=75?w=500&h=500"
            }
            alt="图片"
        ...
        <div className="w-8/12 h-full flex flex-row justify-start items-center">
          <div className="ml-2 max-w-[70%] text-base font-semibold truncate">{curMusic._name}</div>-
          <div className="ml-2 mt-[2px] max-w-[30%] text-sm font-normal truncate">{curMusic._singerName}</div>
        </div>
