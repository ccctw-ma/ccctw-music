...
import { NetEaseIcon, QQMusicIcon, BilibiliIcon, MiGuMusicIcon, OtherIcon } from "../customizeIcons";
...
export default function MusicHomeContent() {
  const musicList = useRecoilValue(musicListState),
    setMusicList = useSetRecoilState(musicListState);
    ...
    <div className="m-2 grid grid-cols-5 gap-1">
      {typeOptions.map((e) => {
        return (
          ...
          >
            <div className="block mb-1 mr-1">{e.icon}</div>
            <div className={`pt-[2px] text-base font-medium ${e.type === type && "text-sky-500"}`}>{e.name}</div>
          </div>
        );
      })}
    </div>
