...
export default function HeaderSongName({ songName, singerName }) {
  ...
  useEffect(() => {
    ...
    if (overflow) {
      ...
      const translateKeyFrames1 =
        "@keyframes songNameMove1" +
        "{" +
        `0% {
      ...
      songNameDom1.current.style.animation =
        "8s linear 0s 1 normal none songNameMove1, 16s linear 8s infinite normal none songNameMove2";
  ...
  return (
    <>
      <div className={`flex flex-row text-lg font-bold rounded-lg align-bottom overflow-hidden`}>
        <span
          className={`block w-full whitespace-nowrap ${!songNameOverflow && "flex flex-row justify-center items-center"}`}
          ref={songNameDom1}
        >
          {songName || "abcdefghijklmnopqrstuvwxyz1234567890+-"}
        </span>
        <span className={`${!songNameOverflow && "hidden"} relative w-full whitespace-nowrap`} ref={songNameDom2}>
          {songName || "abcdefghijklmnopqrstuvwxyz1234567890+-"}
        </span>
      </div>
      <div className="flex flex-row justify-center items-center text-sm font-light">{singerName || "周杰倫"}</div>
    </>
  );
}
