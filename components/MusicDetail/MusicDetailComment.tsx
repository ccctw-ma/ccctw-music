...
export default function MusicDetailComment({ setShowComment }) {
    ...
    (async () => {
      if (pageNo > 1) {
        if (curMusic._from === "netease") {
          ...
          console.log(pageNo);
          const appendCommentData = await API.GET(
            `/api/${curMusic._from}/comments?id=${curMusic._id}&pageNo=${pageNo}&type=0`,
          );
          if (appendCommentData.status === 200) {
  ...
  return (
    <div className="relative flex flex-col w-full h-full px-4 pt-2">
      <div className="flex flex-row  space-x-4" onClick={() => setShowComment(false)}>
        <ArrowLeftOutlined
          style={{
            color: "black",
            fontSize: "1rem",
        ...
        <span className="inline-block text-black text-base align-middle font-semibold">评论({totalCount})</span>
      </div>
      <div
        className="flex-1 flex flex-col w-full h-full overflow-scroll mt-4"
        ref={commentListRef}
              ...
              <div className="flex flex-row w-full">
                ...
                <div className="flex flex-row w-full h-10 pl-3 justify-between items-center">
                  <div className="flex flex-col">
                    <span className="text-base text-gray-600 font-bold">{c._nickName || "已注销"}</span>
                    <span className="text-xs">{c._time}</span>
                  </div>
                  <div className="flex justify-center items-center">
                    <div className="text-black mr-1">{c._praiseCount}</div>
                    <LikeOutlined
                      style={{
                        color: "black",
                        fontSize: "1rem",
