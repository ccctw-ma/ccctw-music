...
export default function MusicDetailCover() {
  ...
  const curMusic = useRecoilValue(curMusicState),
    setCurMusic = useSetRecoilState(curMusicState);
  ...
  useEffect(() => {
    ...
    let content = null;
    if (lyricType === 0) {
      content = <div className="flex flex-1 justify-center items-center text-white text-base">暂无歌词</div>;
    } else if (lyricType === 1) {
    ...
    <div className="relative grow overflow-hidden">
      ...
      >
        <img
          ...
          src={curMusic._coverUrl}
          alt=""
        />
      </div>
      {/* 歌曲歌词 */}
      <div
        className={`absolute w-full h-full flex items-center transition-all duration-500 overflow-hidden ${isCover && "opacity-0 -z-10"}`}
      >
        ...
        >
          <div
            ref={lyricParentRef}
            className="w-full h-full flex flex-col items-center transition-all duration-500 ease-in-out"
