...
export default function MusicDetail({ setIsHome, musicPlayer }) {
  ...
  return (
    <div className="relative w-full h-screen">
      ...
      {/* 显示一些格外的信息 */}
      <div
        className={`absolute w-full h-3/4 left-0 bottom-0 bg-white z-20 rounded-t-3xl transition-all duration-500 ease-in-out shadow-xl
                ${!showComment && "translate-y-full"}`}
      >
        {commentContent}
      </div>
      <div
        className="absolute top-0 left-0 w-screen h-screen bg-cover bg-top  -z-50 blur-[4px] brightness-[.5] saturate-50 "
        style={{
          backgroundImage: `url(${curMusic._coverUrl || "http://mms0.baidu.com/it/u=2999215938,1867739424&fm=253&app=138&f=JPEG&fmt=auto&q=75?w=500&h=500"})`,
        }}
      ></div>
    </div>
  );
