...
- EdgeOne 函数入口放在仓库根目录 `edge-functions/`，需要覆盖 `/health` 和 `/v1/*`，并代理到统一 Cloudflare Worker API，确保国内线路的 API 路径与海外一致；同时保留 `functions/` 轻量跳转兜底，避免 EdgeOne 历史 Node 云函数路由导致代理超时。
