...
- EdgeOne Functions entry points live in the repository root `edge-functions/` directory and cover `/health`, `/v1/*`, and the legacy `/api/*` proxy paths. Lightweight redirect fallbacks are also kept in `functions/` to avoid timeouts from historical EdgeOne Node cloud function routes.
